...
- (void)layoutSubviews {
    ...
    if (_autoFocus && selectedSection>=0 && [self numberOfSectionsInTableView:self.tableView] && [self tableView:self.tableView numberOfRowsInSection:selectedSection]){
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [self.tableView scrollToRowAtIndexPath:indexPath atScrollPosition:self.tableViewScrollPosition animated:_autoFocusAnimate];
        });
    }
}
...
-(void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.selectedBackgroundColor && [item[@"selected"] intValue])
    {
        ...
        if (item[@"transparent"])
        {
            ...
            UIColor *backgroundColor = [RCTConvert UIColor:item[@"backgroundColor"]];
...
-(UITableViewCell* )tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (item[@"userInteractionEnabled"]) {
        cell.userInteractionEnabled = [item[@"userInteractionEnabled"] boolValue];
    }
    return cell;
}
